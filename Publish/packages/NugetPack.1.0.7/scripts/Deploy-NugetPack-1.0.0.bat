...
REM    ***************************************************************************
REM    * 									 *
REM    *  Deploy-NugetPack-1.0.0.bat					 *
REM    * 									 *
REM    * 	Deploys BizTalk Solution using Installed BTDF MSI Package	 *
...
"%INSTALL_DIR%\Deployment\Framework\DeployTools\EnvironmentSettingsExporter.exe" "%INSTALL_DIR%\Deployment\EnvironmentSettings\SettingsFileGenerator.xml" "%INSTALL_DIR%\1.0\Deployment\EnvironmentSettings"


ECHO "Deploying Application..."

"%BTDFMSBuildPath%" /p:DeployBizTalkMgmtDB=%IS_PRIMARY%;Configuration=Server;SkipUndeploy=true;IS_SHARED=%IS_SHARED_ENV%;ENV_SETTINGS="%INSTALL_DIR%\Deployment\EnvironmentSettings\%SETTINGS_FILE%" /target:Deploy /l:FileLogger,Microsoft.Build.Engine;logfile="%INSTALL_DIR%\DeployResults\DeployResults.txt" "%INSTALL_DIR%\Deployment\Deployment.btdfproj" /p:IS_SHARED=%IS_SHARED_ENV% /p:BT_DEPLOY_MGMT=%IS_PRIMARY% /p:ENV_SETTINGS="%INSTALL_DIR%\Deployment\EnvironmentSettings\%SETTINGS_FILE%"
